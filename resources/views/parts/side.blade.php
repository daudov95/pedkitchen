<div class="archive__left">
    ...
    @if(isset($menu))
        <ul class="archive-menu">
            <li class="archive-menu__item"><a href="{{ route('posts', ['category' => $parentCategory]) }}" class="archive-menu__link archive--icon-wishlist {{ !isset($category) ? 'active' : '' }}">Витрина</a></li> 
            ...
            @endforeach
                @if (auth()->check())
                    <li class="archive-menu__item"><a href="{{ route('profile.wishlist') }}" class="archive-menu__link archive--icon-wishlist">Мои избранные рецепты</a></li> 
                @endif
        </ul>
    @endif
